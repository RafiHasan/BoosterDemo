...
$user[0] = DB::table('profile')->select('FirstName','id')->get();
$user[1] = DB::table('basic')->select('projecttitle','shortblurb','fundingduration','fundinggoal','profileid','fundraised','id','projectimage','projectvideo')->get();
    //->where([['Email',$email],['Password',$password]])
    session()->put('staffpick', $user);
...
$id = DB::table('basic')->insertGetId(
        array('projecttitle' => $projecttitle,'shortblurb'=>$shortblurb,'projectlocation'=>$projectlocation,'fundingduration'=>$fundingduration,'fundinggoal'=>$fundinggoal,'profileid'=>$profileid)
    );
